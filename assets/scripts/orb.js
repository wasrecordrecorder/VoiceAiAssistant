  ...
  createProgram() {
    ...
    const fragment = this.createShader(this.gl.FRAGMENT_SHADER, `
      ...
      void main() {
        ...
        float thinking = smoothstep(1.85, 2.35, u_mode) * (1.0 - smoothstep(2.55, 3.0, u_mode));
        float speed = 0.18 + active * 0.28 + thinking * 0.22 + speaking * 0.35 + energy * 0.42;
        float pulse = sin(t * (0.82 + active * 0.95 + energy * 1.25)) * 0.011 + sin(t * 2.7 + energy * 3.2) * 0.004;
        pulse += energy * (0.03 + speaking * 0.045) + thinking * sin(t * 1.7) * 0.006;
        vec2 q = uv * (2.62 - energy * 0.1 - speaking * 0.04);
        float warp = fbm(q + vec2(t * speed, -t * speed * 0.54));
        float ring = sin(atan(uv.y, uv.x) * 6.0 + t * (1.25 + energy * 2.2));
        float wave = sin(q.x * 4.0 + t * 1.32 + warp * 4.0) * cos(q.y * 3.2 - t * 0.83 + warp * 3.1);
        float radius = 0.322 + pulse + wave * (0.008 + energy * 0.034 + active * 0.005) + ring * energy * 0.012;
        float d = length(uv) - radius;
        float body = 1.0 - smoothstep(-0.014, 0.013, d);
        float halo = exp(-max(d, 0.0) * (13.0 - energy * 2.0)) * (0.12 + active * 0.08 + energy * 0.16 + speaking * 0.08);
